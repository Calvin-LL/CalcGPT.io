import { fetchBin, fetchJson } from "./fetch";
import {
  AttentionBlock,
  DeEmbedBlock,
  EmbedBlock,
...
import { GPT2Tokenizer, type Tokenizer } from "./tokenizer";
...
export class GPT {
  ...
  tokenizer?: Tokenizer;
  params?: Params;
  minBufferOffset: number;
  unloadDeletionStack: GPUBuffer[];
  computePasses?: any[];

  constructor(folder: string = "gpt2-weights", type: string = "bpe") {
    this.folder = folder;
  ...
  async *generate(
    ...
    for (let i = 0; i < max_new_tokens; i++) {
      ...
      const result = this.tokenizer!.decode([idx_next]);
      if (result === "<|endoftext|>" || result.includes(stopSequence)) {
        break;
      }
    ...
    //     totalTime / (max_new_tokens - warmupRuns)
    //   } ms`,
    // );
  }
  ...
  async loadModel(folder: string) {
    if (this.initialized) {
      throw new Error("Model already loaded");
    }

    // console.log("Loading model from folder:", folder);
    const weightsFolder = folder;
  ...
  async loadParameters(weightsFolder: string) {
    // console.log("Loading params...");
    const params = (await fetchJson(
      `${weightsFolder}/params_gpt.json`,
    )) as Params;
  ...
  async loadEmbeddings(params: Params, weightsFolder: string) {
  ...
  async loadPositionalEmbeddings(params: Params, weightsFolder: string) {
  ...
  async loadFinalLayerNorm(params: Params, weightsFolder: string) {
  ...
  async loadLayers(params: Params, weightsFolder: string) {
  ...
  async loadLayer(params: Params, weightsFolder: string, layerIndex: number) {
  ...
  async fetchAndSplitQKVWeightTensors(
    url: string,
    dims: number[],
    ops: (keyof typeof bufferUsageDict)[],
  ) {
  ...
  async fetchAndSplitQKVBiasTensors(
    url: string,
    dims: number[],
    ops: (keyof typeof bufferUsageDict)[],
  ) {
  ...
  bufferSize(dimX: number, dimY = 1, dimZ = 1) {
    const size =
      Math.ceil(
