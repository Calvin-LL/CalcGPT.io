...
import { bufferUsageDict } from "./globals";
import { wgSize } from "./helpers";
...
export class Block {
  name: string = "";
  device?: GPUDevice;
  pipelineCache: Map<string, GPUComputePipeline> | null = new Map();
  boundBufferCache: Map<string, GPUBuffer[]> = new Map();
  unboundBufferCache: Map<string, GPUBuffer[]> = new Map();
  r_r_r_r_Layout?: GPUBindGroupLayout;
  r_r_r_Layout?: GPUBindGroupLayout;
  r_r_Layout?: GPUBindGroupLayout;
  r_Layout?: GPUBindGroupLayout;
  u_s_Layout?: GPUBindGroupLayout;
  u_s_s_s_Layout?: GPUBindGroupLayout;
  ...
  initBindGroup(layout: GPUBindGroupLayout, buffers: GPUBuffer[], label = "") {
  ...
  initUniform(size: number, values: [number, Uint32Array | Float32Array][]) {
  ...
  initBuffer(ops: (keyof typeof bufferUsageDict)[], dims: number[], key = "") {
    const buffer = this.device!.createBuffer({
      size: this.bufferSize(dims[0], dims[1] || 1),
  ...
  initBindGroups() {
    const bg = (types: GPUBufferBindingType[]) =>
...
export class FastMatMulBlockClass extends Block {
  constructor() {
    super();
    this.name = "fastMatMul";
  }

  getPipeline(rows: number) {
  ...
  fastMatMul(flag: "withCheck" | "noCheck") {
    const outputCode = {
      withCheck: `
      if (y * 4u + 0u < M) {
...
export class ResidualBlockClass extends Block {
  constructor() {
    super();
    this.name = "residual";
  }
...
export class LayerNormBlockClass extends Block {
  constructor() {
    super();
    this.name = "layerNorm";
  }

  getStatsPipeline(workgroups: number) {
  ...
  normStatsShader = (wg_size: number) => `
    struct Meta {
      N: u32, 
    };
...
export class SoftmaxBlockClass extends Block {
  constructor() {
    super();
    this.name = "softmax";
  }

  getFusedPipeline(workgroups: number, transpose: boolean) {
  ...
  */
  fusedShader(wg_size: number, transpose: boolean) {
...
export class GeluBlockClass extends Block {
  constructor() {
    super();
    this.name = "gelu";
  }
  ...
  newInstance(rows: number, cols: number, inputBuf: GPUBuffer) {
    if (cols % 4 !== 0) throw new Error("Cols must be divisible by 4.");
    const pipeline = this.getPipeline();
    const uniformBuffer = this.initUniform(4, [
      [0, new Uint32Array([rows, Math.ceil(cols / 4)])],
...
export class EmbedBlockClass extends Block {
  constructor() {
    super();
    this.name = "embed";
  }
  ...
  staticLoad(
    embdOutputBuffer: GPUBuffer,
    posEmbdOutputBuffer: GPUBuffer,
    embdBuffers: GPUBuffer[],
    posEmbdBuffer: GPUBuffer,
    idx: number[],
    seq_length: number,
    n_embd: number,
  ...
  newInstance(
    ...
    embdBuffers: GPUBuffer[],
    posEmbdBuffer: GPUBuffer,
    ResidualBlock: ResidualBlockClass,
  ) {
...
export class DeEmbedBlockClass extends Block {
  constructor() {
    super();
    this.name = "deembed";
  }
...
export class AttentionBlockClass extends Block {
  constructor() {
    super();
    this.name = "attention";
  }
  ...
  newFusedInstance(
    seq_length: number,
    n_embd: number,
    attentionDotProductScale: number,
    n_head: number,
    head_size: number,
    inputBuffer: GPUBuffer,
    qWeightsBuffer: GPUBuffer,
    qBiasBuffer: GPUBuffer,
    kWeightsBuffer: GPUBuffer,
    kBiasBuffer: GPUBuffer,
    vWeightsBuffer: GPUBuffer,
    vBiasBuffer: GPUBuffer,
    linearWeightsBuffer: GPUBuffer,
    linearBiasBuffer: GPUBuffer,
    FastMatMulBlock: FastMatMulBlockClass,
    SoftmaxBlock: SoftmaxBlockClass,
  ) {
